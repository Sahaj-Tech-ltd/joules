  ...
  <div class="flex min-h-screen flex-col bg-slate-950">
    <div class="flex flex-col items-center px-6 pt-8 pb-4">
      <div class="flex items-center gap-3">
        <Logo size={36} />
        <span class="text-xl font-bold text-white">Joules</span>
      </div>
    </div>
    ...
    <div class="mx-auto w-full max-w-lg flex-1 px-6 pb-8">
              ...
              <div class="grid grid-cols-1 gap-3">
                {#each dietPlanOptions as option}
                  ...
                  >
                    ...
                    <div class="mt-1 text-xs font-medium text-joule-400">{option.bestFor}</div>
                    <div class="mt-1 text-xs text-slate-400">{option.tip}</div>
                  </button>
                {/each}
              </div>
            </div>
      ...
      {#if currentStep === 5}
        <div>
          <h2 class="text-2xl font-bold text-white">Your Food Preferences</h2>
          <p class="mt-1 text-sm text-slate-400">Optional — helps your AI coach give better advice</p>

          <div class="mt-8 space-y-6">
            <div>
              <span class="mb-2 block text-sm font-medium text-slate-300">Diet Type</span>
              <div class="flex flex-wrap gap-2">
                {#each dietTypeOptions as option}
                  <button
                    type="button"
                    onclick={() => (dietType = option.value)}
                    class="rounded-lg border px-3.5 py-2 text-sm font-medium transition {dietType === option.value ? 'border-joule-500 bg-joule-500/10 text-joule-400' : 'border-slate-700 text-slate-400 hover:border-slate-600'}"
                  >
                    {option.label}
                  </button>
                {/each}
            ...
            <div>
              <span class="mb-2 block text-sm font-medium text-slate-300">Allergies / Intolerances</span>
              <div class="flex flex-wrap gap-2">
                {#each allergyOptions as option}
                  <button
                    type="button"
                    onclick={() => toggleAllergy(option.value)}
                    class="rounded-lg border px-3.5 py-2 text-sm font-medium transition {allergies.includes(option.value) ? 'border-joule-500 bg-joule-500/10 text-joule-400' : 'border-slate-700 text-slate-400 hover:border-slate-600'}"
                  >
                    {option.label}
                  </button>
                {/each}
            ...
            <div>
              <label for="food-notes" class="mb-1.5 block text-sm font-medium text-slate-300">Tell us about your food preferences</label>
              <textarea
                id="food-notes"
                bind:value={foodNotes}
                rows="3"
                placeholder="e.g. I love pizza, usually eat veggie burgers on weekdays"
                class="w-full rounded-lg border border-slate-700 bg-surface-light px-3.5 py-2.5 text-sm text-white placeholder:text-slate-500 focus:border-joule-500 focus:outline-none focus:ring-1 focus:ring-joule-500 resize-none"
              ></textarea>
            </div>

            <div>
              <label for="eating-context" class="mb-1.5 block text-sm font-medium text-slate-300">Where / how do you usually eat?</label>
              <textarea
                id="eating-context"
