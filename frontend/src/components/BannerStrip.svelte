<script lang="ts">
  ...
  const typeStyles: Record<string, { bg: string; border: string; text: string; icon: string }> = {
    ...
    tip:     { bg: 'bg-joule-500/10', border: 'border-joule-500/20', text: 'text-joule-400', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z' },
    ...
    {#each banners as banner}
      ...
      <div class="flex items-start gap-3 rounded-xl border px-4 py-3 {style.bg} {style.border}">
        ...
        <div class="flex-1 min-w-0">
          ...
          {/if}
          <p class="text-sm text-slate-300 {banner.title ? 'mt-0.5' : ''}">{banner.message}</p>
        </div>
        <button
          onclick={() => dismiss(banner.id)}
          class="shrink-0 text-slate-500 hover:text-white transition"
          aria-label="Dismiss"
        >
