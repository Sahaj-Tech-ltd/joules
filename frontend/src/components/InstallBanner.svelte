...
{#if visible}
  ...
  <div
    class="fixed bottom-16 left-3 right-3 z-40 rounded-2xl border border-primary/30 bg-secondary shadow-2xl shadow-black/40 p-4"
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-sm font-semibold text-foreground">Add Joules to your home screen</p>
        {#if platform === 'ios'}
          <p class="mt-0.5 text-xs text-foreground leading-relaxed">
            Tap <span class="inline-flex items-center gap-0.5 text-primary font-medium">
              <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
              </svg>
              Share
            </span> then <strong class="text-foreground">"Add to Home Screen"</strong>
          </p>
        {:else if platform === 'android-chrome'}
          <p class="mt-0.5 text-xs text-foreground">Get the full app experience with one tap.</p>
        {/if}
      </div>
      ...
      <div class="flex items-center gap-2 shrink-0">
        ...
        <button
          onclick={dismiss}
          class="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent/50 transition"
          aria-label="Dismiss"
        >
