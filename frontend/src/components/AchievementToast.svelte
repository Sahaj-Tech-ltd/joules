...
{#if browser}
  ...
  >
    {#each toasts as toast (toast.id)}
      <div
        class="pointer-events-auto flex items-start gap-3 rounded-xl border border-amber-500/30 bg-slate-900 px-4 py-3.5 shadow-2xl shadow-black/40 transition-all duration-500"
        style="
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          <p class="text-xs font-semibold uppercase tracking-wider text-amber-400">Achievement Unlocked</p>
          <p class="mt-0.5 text-sm font-bold text-white leading-tight">{toast.title}</p>
          {#if toast.description}
            <p class="mt-1 text-xs text-slate-400 leading-snug">{toast.description}</p>
          {/if}
        </div>
        ...
        <button
          ...
          class="mt-0.5 shrink-0 rounded p-0.5 text-slate-500 hover:text-slate-300 transition"
          aria-label="Dismiss"
        >
